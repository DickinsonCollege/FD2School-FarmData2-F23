describe("Test the harvest report table values", () => {
    beforeEach(() => {
    ...
    it("Check filtering by crop", () => {
        cy.get("[data-cy=crop-drop] > [data-cy=dropdown-input]").select("ARUGULA")

        cy.get("[data-cy=generate-report-button]").click()

        cy.get("[data-cy=table-body").children().should("have.length","4")

        cy.get("[data-cy=td-r0c3").contains("ARUGULA")

        cy.get("[data-cy=td-r1c3").contains("ARUGULA")

        cy.get("[data-cy=td-r2c3").contains("ARUGULA")
